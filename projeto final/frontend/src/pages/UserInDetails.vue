...
async function updateUser() {
  try {
    loading.value = true
    const res = await api.put(`http://localhost:2222/users/${user.value.id}`, {
...
async function addUser() {
  try {
    loading.value = true
    const res = await api.post(`http://localhost:2222/users/`, {
      name: user.value.name,
      username: user.value.username,
      email: user.value.email,
...
async function loadUser(id: Number) {
  try {
    const res = await api.get(`http://localhost:2222/users/${id}`, {
      headers: {
        Authorization: `Bearer ${userStore.jwt}`
      }
    })
...
async function loadRoles() {
  try {
    const res = await api.get(`http://localhost:2222/roles`, {
      headers: {
        Authorization: `Bearer ${userStore.jwt}`
      }
    })
